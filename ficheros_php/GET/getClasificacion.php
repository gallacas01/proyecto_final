...
SUM(CASE WHEN p.id_equipo_local = e.id_equipo AND p.goles_local > p.goles_visitante THEN 3 
         ...
         WHEN p.id_equipo_local = e.id_equipo AND p.goles_local = p.goles_visitante THEN 1 
         WHEN p.id_equipo_visitante = e.id_equipo AND p.goles_visitante = p.goles_local THEN 1 
         ELSE 0 
    END) AS puntos,
    c.temporada
FROM equipo e 
INNER JOIN partido p ON e.id_equipo = p.id_equipo_local OR e.id_equipo = p.id_equipo_visitante, competicion c
